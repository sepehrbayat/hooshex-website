    ...
    <form wire:submit.prevent="callMountedAction">
        ...
        <x-filament::modal
            ...
            x-on:modal-closed.stop="
                ...
                // FIX: Use requestAnimationFrame for stronger timing guarantee
                // Ensures Livewire has fully completed DOM morphing before unmount
                requestAnimationFrame(() => {
                    setTimeout(() => {
                        try {
                            const livewireEl = $el.closest('[wire\\:id]')
                            if (!livewireEl || !$wire) return
                            
                            // Check DOM existence first (cheaper check)
                            if (!document.body.contains(livewireEl)) return
                            
                            // Then check function availability
                            if (typeof $wire.unmountAction !== 'function') return
                            
                            $wire.unmountAction(false, false)
                        } catch (e) {
                            // Ignore error if component was removed during event handling
                        }
                    }, 0)
                })
            "
                ...
                requestAnimationFrame(() => {
                    setTimeout(() => {
                        try {
                            ...
                            // Then check function availability
                            if (typeof $wire.unmountTableAction !== 'function') return
                ...
                requestAnimationFrame(() => {
                    setTimeout(() => {
                        try {
                            ...
                            // Then check function availability
                            if (typeof $wire.unmountTableBulkAction !== 'function') return
                ...
                requestAnimationFrame(() => {
                    setTimeout(() => {
                        try {
                            ...
                            // Then check function availability
                            if (typeof $wire.unmountInfolistAction !== 'function') return
                    ...
                    requestAnimationFrame(() => {
                        setTimeout(() => {
                            try {
                                ...
                                // Then check function availability
                                if (typeof $wire.unmountFormComponentAction !== 'function') return
