...
<div
    ...
    x-data="{
        ...
        close: function () {
            // FIX: Guard against race condition where Livewire removes component before close
            try {
                if (!this.$el || !document.body.contains(this.$el)) return
                
                this.isOpen = false

                if (this.$refs.modalContainer && document.body.contains(this.$refs.modalContainer)) {
                    this.$refs.modalContainer.dispatchEvent(
                        new CustomEvent('modal-closed', { detail: { id: '{{ $id }}' } }),
                    )
                }
            } catch (e) {
                // Silently ignore if component was removed during event handling
            }
        },

        open: function () {
            this.$nextTick(() => {
                // FIX: Guard against race condition
                try {
                    if (!this.$el || !document.body.contains(this.$el)) return
